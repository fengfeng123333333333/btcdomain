...
const mobileLink =
  domain.domainImgUrl + "assets/banner_getwebsite_mobile@2x.png";
...
async function submitBtcTxAction() {
  ...
  if (one.gte(avail)) {
    ElMessage.warning(
      "max value you must transfer is " + stat.sendInsOrBtc.availBal + "btc"
    );
    return;
  }
  // feeRate
...
function cardShareAction() {
  ...
  domtoimage.toJpeg(node, { quality: 0.95 }).then(function (dataUrl: any) {
...
function loadavatar() {
  ...
  if (addr) {
    ...
    console.log("stat.pinfo.address", stat.pinfo.address);
    ...
    service.avatarGet(addr).then((avatarRet) => {
      if (avatarRet.data.length > 0) {
        ...
        if (stat.pinfo.domain) {
          stat.bCard.qrlink =
            "https://app.btcdomains.io/#/?search=" + stat.pinfo.domain;
        }
      }
    });
  }
...
function assembleIcons() {
  stat.bCard.icons.forEach((element) => {
    element.file_sel =
      domain.domainImgUrl + "assets/bcard/" + element.name + "_sel@2x.png";
    element.file_dis =
      domain.domainImgUrl + "assets/bcard/" + element.name + "_dis@2x.png";
  });
...
<template>
  <div class="wallet-container">
      ...
      <div class="top-inner-view">
        <div class="info-view">
          <div class="topwarp-view">
            <img class="avatar-view" :src="stat.pinfo.content_url ? stat.pinfo.content_url : defaultAvatar" alt="">
            <div class="nick-addr-view">
              ...
              <div class="addrname-view">{{ stat.pinfo.short_addr }}
                <img src="../assets/icon_copy_white@2x.png" style="width: 20px;height: 20px;cursor: pointer;margin-left: 5px;" alt="" @click="copyAction">
                <img src="../assets/icon_qrcode@2x.png" style="width: 20px;height: 20px;cursor: pointer;;margin-left: 5px;" alt="" @click="showQrCodeAction">
              </div>
            </div>
          </div>
        ...
        <div class="assets-view">
          <div class="nums-view">
            ...
            <div style="margin-left: 10px;">
              <div class="btc-view">{{ stat.winfo.amount }} BTC <img src="../assets/icon_q@2x.png" alt="" width="24" height="24"></div>
              <div class="usdt-view">≈ {{ stat.winfo.usd_value }} USDT</div>
            </div>
          </div>

          <div class="actions-view">
            <div class="action-view send-one" @click="sendBtcsAction">Send<img src="../assets/icon_send@2x.png" alt="" width="24" height="24"></div>
            <div class="action-view receive-one" @click="receiveAction">Receive <img src="../assets/icon_receive@2x.png" alt="" width="24" height="24"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <el-dialog v-model="stat.sendInsOrBtc.isSendInsOrBtcShow" :show-close="true" :align-center="true" :width="stat.dialogueWidth" class="send-dialogue-view">
      <template #header="{ close, titleId, titleClass }">
        <div class="my-header">
      ...
      <div style="padding-left: 30px;padding-right: 30px;">
        <div class="to-view">
          ...
          <el-input v-model="stat.sendInsOrBtc.toAddr" placeholder="Received Bitcoin address or .btc domain name" class="to-addr-input" @input="addressChange" />
        ...
        <div class="fee-summary-view">
          <div class="fee-card-view fee-card-dif-view" v-for="(item, idx) in stat.sendInsOrBtc.feeSums.list" :key="idx" :class="stat.sendInsOrBtc.curIdx == idx ? 'fee-card-view-selected' : 'fee-card-view-normal'"
        ...
        <div class="card-view" id="personal-card-view">
          <div class="card-top-view">
            <div class="card-avatar-view">
              <img class="card-avatar-img" referrerpolicy="no-referrer" :src="stat.pinfo.content_url ? stat.pinfo.content_url : defaultAvatar" width="80" height="80" alt="">
              <img class="card-avatar-og" v-if="stat.bCard.isOg" referrerpolicy="no-referrer" :src="stat.bCard.ogLink" width="30" height="30" alt="">
            </div>
            <div class="card-content-view">
              ...
              <div class="card-icon-view">
                <div v-for="(item, idx) in stat.bCard.icons" :key="idx">
                  <img referrerpolicy="no-referrer" :src="item.isHighlight ? item.file_sel : item.file_dis" width="18" height="18" alt="">
                </div>
              </div>
            </div>
          </div>
