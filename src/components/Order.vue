...
function lockFeeAndQueryBalance(loadingIns: any) {
  service
    .lockFee(state.info.name, state.info.years, state.info.walletId)
    .then((val4) => {
      service.queryBalance(state.info.walletId).then((val5) => {
        ...
        state.info.gasFee = g_fee.toPrecision(Types.precision).toString();
        state.info.serviceFee = s_fee.toPrecision(Types.precision).toString();
        state.info.registerFee = reg_fee
          .toPrecision(Types.precision)
          .toString();
        state.info.balance = b_fee.equals(0)
          ? "0"
          : b_fee.toPrecision(Types.precision).toString();
...
onMounted(() => {
  ...
  useRequest(queryAction, {
    ...
    onSuccess: (val1) => {
      ...
      state.info.serviceFee = val1.data.service_fee.toPrecision(
        Types.precision
      );
      state.info.origin_service_fee = val1.data.origin_service_fee.toPrecision(
        Types.precision
      );
